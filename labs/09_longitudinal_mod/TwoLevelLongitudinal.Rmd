...
chart.wide <- read_csv("class _projects/chart_wide_condense.csv")
```
...
regressions.int <- chart.long %>% 
  group_by(schoolid) %>% 
  do(int = lm(MathAvgScore ~ year08, data=.)$coefficients[1]) %>%
  mutate(int = as.numeric(int)) 

regressions.slope <- chart.long %>% 
  group_by(schoolid) %>% 
  do(rate = lm(MathAvgScore ~ year08, data=.)$coefficients[2]) %>%
  mutate(rate = as.numeric(rate))

regressions <- left_join(regressions.int, regressions.slope)

lm_info <- left_join(regressions, chart.wide)
...
lon.histmat1 <- grid.arrange(int.hist1, rate.hist1,  ncol=2)
