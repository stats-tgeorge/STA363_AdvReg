---
title: 'Ch8: Introduction to Multilevel Models'
output:
  pdf_document:
    fig_height: 3
    fig_width: 4.5
...
music <- read_csv("~/Stats 316 F19/Class/Data/musicdata.csv")
```
...
#Code for bivariate normal plots
...
mc <- update(model.c,control=lmerControl(optimizer="bobyqa",
...
confint(model.f)  
# get error with default profile likelihood method

confint(model.f, method = "Wald")  
# only does fixed effects

confint(model.f, method = "boot", boot.type = "perc", nsim = 100)  
# runs in 10 minutes or so with default 500 sims
...
mli.density1 <- grid.arrange(a0dens,a1dens,b0dens,b1dens,ncol=2)
...
b1dens <- ggplot(data=se.data, aes(x=b1,linetype=model,group=model)) + 
  ...
  ylab("Density") + labs(title="(d)")
mli.density2 <- grid.arrange(a0dens,a1dens,b0dens,b1dens,ncol=2)
```
